...
- **Gacha**: catalog in `CosmeticCatalog.cs`. `EquippedFrameKey`/`EquippedThemeKey`/`AvatarEmoji`/`Dust`/`PullsSinceLegendary` on `User`. Equipping a Color cosmetic writes `User.AvatarColor = def.Value`; equipping an Emoji cosmetic writes `User.AvatarEmoji = def.Value` (a null key clears it, avatar falls back to initials) — both denormalized, thread via `UserDto.FromEntity`/`LeaderboardEntryDto`. The `Avatar` component (`ui/Modal.tsx`) renders `emoji` in place of initials wherever it renders `frame`. `color-indigo` (#6366f1) is the one `Default = true` cosmetic (free for everyone); Emoji has no default (initials are the fallback). Equipping lives in `CustomizationModal.tsx`, not GachaPage. Shared refetch helper: `syncAppearanceFromServer` in `lib/appearance.ts`.
...
- **Reassignment**: `ChoreService.ReassignAsync` branches on the actor's role. An **admin** moves the chore immediately (logs a `ChoreAssignment` reassignment row). A **member** may only reassign a chore they currently hold; their request does **not** move the chore — it records a `ChoreReassignmentRequest { ChoreId, FromUserId, ToUserId, Status }` (`ReassignmentStatus.Pending`) and notifies the target via `NotificationService.NotifyUserAsync` (in-app inbox + Web Push + FCM). At most one Pending request exists per chore (a newer request `Cancelled`s the older; admin reassign / freeze rotation also cancel pending). The target responds via `RespondToReassignmentAsync` (`POST /api/chores/{id}/reassign/{accept|decline}`, caller must be the request's `ToUser`): **accept** sets `CurrentAssigneeId` + logs the `ChoreAssignment`, **decline** leaves it. The active Pending request surfaces on `ChoreDto.PendingReassignment` (loaded per chore in List/Get via `PendingReassignmentsByChoreAsync`). Frontend: `ChoreMenu` gates the Reassign item (`isAdmin || currentAssignee === meId`); `ReassignModal` rewords for members; `ChoreListItem`/`ChoreCompactItem` show a compact "Reassign pending"/"Reassign request" badge only. The **Accept/Decline action lives in `ChoreDetailsModal`** (where the notification deep-link `/chores?chore={id}` lands) — it self-sources `meId` from the auth store and calls `accept/declineReassign`, invalidating `['chores']` and closing on success.
- **Capacitor (Android)**: `isNative()` in `lib/native.ts` is the single switch. Three web assumptions lifted on native: (1) API base URL uses stored origin from `@capacitor/preferences`; (2) refresh token returned in response body (`X-Turnly-Client: android`) and read from `X-Refresh-Token` header; (3) CORS allows `https://localhost`. FCM push via `FcmSender` (inert when unconfigured). The native WebView has no `serviceWorker`/`PushManager`, so `SettingsPage` renders a separate `NativeNotificationsCard` (FCM permission + enable/disable) instead of the Web Push `NotificationsCard`; `lib/native-push.ts` exposes `nativePushPermission()` and an idempotent `registerNativePush` (listeners attach once, safe to re-call from settings). Web path is byte-for-byte unchanged.
- **Fresh start**: `ResetService.FreshStartAsync` deletes ChoreCompletions, ChoreAssignments, ChoreReassignmentRequests, PointsLog, Redemptions, UserAchievements, UserCosmetics, UserNotifications then zeroes user state. Chore schedules untouched.
...
- `Entities/` — POCOs; `Guid Id = Guid.NewGuid()` + `DateTimeOffset CreatedAt`, no base class.
  Key entities: `User, Chore, ChoreCompletion, ChoreAssignment, ChoreAssigneeTrack,
  ChoreReassignmentRequest, PointsLogEntry, Award, Redemption, ChoreNotification, PushSubscription,
  FcmDevice, NotificationDelivery, UserNotification, AppSetting, UserAchievement, UserCosmetic`.
...
- `components/chores/ChoreRail.tsx` — right-hand info rail on `ChoresPage`, desktop `xl`+ only (sticky, hidden below `xl`; shown in every view including calendar). Two widgets: "Your snapshot" (fresh `me` points/weekly + your overdue/due-today counts derived from the loaded chore list) and weekly leaderboard (top 5, rows open `UserDetailsModal`). Pure presentation over existing queries; no backend changes. `ChoresPage` runs full-width at `xl` (via `Layout`'s route check) so the rail anchors to the screen edge and the chore column centers in the space left of it (`max-w-2xl`); below `xl` it falls back to the standard centered `max-w-5xl` column with the rail hidden.
